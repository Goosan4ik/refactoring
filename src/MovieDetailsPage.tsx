    ...
    const fetchHallPlanAndTickets = async () => {
      try {
        ...
        const [planRes, ticketsRes] = await Promise.all([
          axios.get(API_ENDPOINTS.HALLS.PLAN(selectedSession.hallId)),
          axios.get(API_ENDPOINTS.SESSIONS.TICKETS(selectedSession.id)),
        ]);
  ...
  const handleReserve = async () => {
    ...
    try {
      for (const seatId of selectedSeats) {
        ...
        await axios.post(
          API_ENDPOINTS.TICKETS.RESERVE(ticket.id),
          {},
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const ticketsRes = await axios.get(
        API_ENDPOINTS.SESSIONS.TICKETS(selectedSession?.id || "")
      );
      setTickets(ticketsRes.data);
    } catch (err) {

        ...
        const response = await axios.get(
          `${API_BASE_URL}${API_ENDPOINTS.SESSIONS}`,
          {
            params: {
              ...
              size: PAGINATION.SESSIONS_PAGE_SIZE,
              filmId: movie.id,
            },
          }
        );
    ...
    const fetchHallPlanAndTickets = async () => {
      try {
        ...
        const [planRes, ticketsRes] = await Promise.all([
          axios.get(
            `${API_BASE_URL}${API_ENDPOINTS.HALLS}/${selectedSession.hallId}/plan`
          ),
          axios.get(
            `${API_BASE_URL}${API_ENDPOINTS.SESSIONS}/${selectedSession.id}${API_ENDPOINTS.TICKETS}`
          ),
        ]);
  ...
  const handleReserve = async () => {
    ...
    try {
      for (const seatId of selectedSeats) {
        ...
        await axios.post(
          `${API_BASE_URL}${API_ENDPOINTS.TICKETS}/${ticket.id}/reserve`,
          {},
        ...
        <div className="row g-4 align-items-start">
          <div className="col-md-4 text-center">
            <img
              ...
              alt={movie.title}
              style={{ width: UI_SIZES.POSTER_WIDTH }}
            />
          </div>
              ...
              <div className="mt-4">
                ...
                {hallPlan && (
                  <div
                    ...
                    style={{ gap: UI_GRID.GAP_BETWEEN_ROWS }}
                  >
                    <div className="d-flex flex-wrap justify-content-center mb-3" style={{ gap: UI_GRID.GAP_BETWEEN_CATEGORIES }}>
                      ...
                      {hallPlan.categories.map((c) => (
                        ...
                        >
                          <span
                            className="btn"
                            style={{
                              width: UI_SIZES.LEGEND_ITEM_WIDTH,
                              height: UI_SIZES.LEGEND_ITEM_HEIGHT,
                              padding: 0,
                              backgroundColor: c.name
                                .toLowerCase()
                                .includes("vip")
                                ? UI_COLORS.VIP_COLOR
                                : UI_COLORS.DEFAULT_COLOR,
                              border: c.name
                                .toLowerCase()
                                .includes("vip")
                                ? UI_COLORS.VIP_BORDER
                                : UI_COLORS.DEFAULT_BORDER,
                            }}
                          ></span>
                      ...
                      .map((rowNum) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              display: "grid",
                              gridTemplateColumns: `repeat(${rowSeats.length}, ${UI_SIZES.SEAT_WIDTH})`,
                              gap: UI_GRID.GAP_BETWEEN_SEATS,
                            }}
                          >
                            {rowSeats.map((seat) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`btn ${color}`}
                                  style={{
                                    width: UI_SIZES.SEAT_WIDTH,
                                    height: UI_SIZES.SEAT_HEIGHT,
                                  }}

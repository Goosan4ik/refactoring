...
const MovieDetailsPage: React.FC<Props> = ({ movie, onBack }) => {
    ...
    const fetchSessions = async () => {
      try {
        ...
        const response = await apiClient.get("/sessions", {
          params: { page: DEFAULT_PAGE, size: SESSIONS_PAGE_SIZE, filmId: movie.id },
        });
    ...
    const fetchHallPlanAndTickets = async () => {
      try {
        ...
        const [planRes, ticketsRes] = await Promise.all([
          apiClient.get(`/halls/${selectedSession.hallId}/plan`),
          apiClient.get(`/sessions/${selectedSession.id}/tickets`),
        ]);
  ...
  const getCategory = (catId: string) => hallPlan?.categories.find((c) => c.id === catId);
  ...
  const handleReserve = async () => {
    ...
    try {
      for (const seatId of selectedSeats) {
        ...
        await apiClient.post(`/tickets/${ticket.id}/reserve`, {});
      }
      ...
      const purchaseRes = await apiClient.post(`/purchases`, { ticketIds: reservedTickets });
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const ticketsRes = await apiClient.get(`/sessions/${selectedSession?.id}/tickets`);
      setTickets(ticketsRes.data);
    } catch (err) {
      ...
      <div className="container py-5">
        ...
        <div className="row g-4 align-items-start">
          <div className="col-md-4 text-center">
            <img
              src={movie.imageUrl || MOVIE_POSTER_PLACEHOLDER_URL}
          ...
          <div className="col-md-8">
            ...
            <div className="mb-3">
              ...
              <input
                ...
                style={{ width: `${DATE_INPUT_WIDTH_PX}px` }}
                value={selectedDate}
                onChange={(e) => {
            ...
            <div className="d-flex flex-wrap gap-2 mt-2">
              ...
              {!loadingSessions && filteredSessions.length === 0 && <p>Сеансов нет</p>}
              {!loadingSessions &&
                ...
                {purchase && hallPlan && (
                  <div className="text-center mt-4 p-3 border border-light rounded">
                    ...
                    <div className="d-flex flex-column align-items-center gap-2">
                      ...
                      />
                      <button className="btn btn-success px-5 mt-2" onClick={handlePayment}>
                        Оплатить
                      </button>
                    </div>
                  </div>

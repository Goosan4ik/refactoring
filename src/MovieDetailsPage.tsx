...
import { requireAuth, checkAuthForPayment } from "./utils/authUtils";
...
const MovieDetailsPage: React.FC<Props> = ({ movie, onBack }) => {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        ...
        handleApiError(err, "загрузки сеансов");
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchHallPlanAndTickets = async () => {
      try {
        ...
        handleApiError(err, "загрузки плана или билетов");
      } finally {
  ...
  const handleReserve = async () => {
    if (!requireAuth("Сначала авторизуйтесь")) return;
    ...
    try {
      ...
      const purchaseRes = await apiClient.post("/purchases", {
        ticketIds: reservedTickets,
      });

      setPurchase(purchaseRes.data);
    } catch (err) {
      handleApiError(err, "при бронировании или покупке");
      alert("Ошибка бронирования. Проверьте авторизацию и доступность мест.");
    }
  };

  const handlePayment = async () => {
    if (!checkAuthForPayment() || !purchase) return;
    ...
    try {
      ...
      const ticketsRes = await apiClient.get(
        `/sessions/${selectedSession?.id}/tickets`
      );
      setTickets(ticketsRes.data);
    } catch (err) {
      handleApiError(err, "оплаты");
